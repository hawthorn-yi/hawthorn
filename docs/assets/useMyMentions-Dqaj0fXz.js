import{r as o}from"./react-vendor-yb0GR-CG.js";import{s as i}from"./index-CK0XXMGy.js";import{t as I}from"./ui-vendor-p-tr_I-7.js";function q(){const[m,p]=o.useState([]),[h,k]=o.useState([]),[M,C]=o.useState(!0),[d,v]=o.useState(null);o.useEffect(()=>{i.auth.getSession().then(async({data:r})=>{const n=r.session?.user?.id||null;v(n)})},[]);const l=o.useCallback(r=>{const n=new Map;for(const e of r){n.has(e.progress_entry_id)||n.set(e.progress_entry_id,{progress_entry_id:e.progress_entry_id,task_id:e.task_id,task_name:e.task_name,note:e.note,created_at:e.created_at,mentioned_users:[],replies:[],has_reply:!1,dismissed:!1});const t=n.get(e.progress_entry_id);t.mentioned_users.includes(e.mentioned_username)||t.mentioned_users.push(e.mentioned_username);for(const s of e.replies)t.replies.some(f=>f.id===s.id)||t.replies.push(s);e.has_reply&&(t.has_reply=!0),e.dismissed&&(t.dismissed=!0)}const a=Array.from(n.values()).sort((e,t)=>new Date(t.created_at).getTime()-new Date(e.created_at).getTime());k(a)},[]),_=o.useCallback(async()=>{if(d)try{const{data:r,error:n}=await i.from("notifications").select(`
          id,
          from_user_id,
          to_user_id,
          task_id,
